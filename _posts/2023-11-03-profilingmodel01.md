---
layout: single
title:  "Part 01. 온라인 메시지 작성자 프로파일링 모델 개발: 데이터 기초 탐색"
...
⇒ 일반적인 불용어 제거 및 표제화에서 제거되는 표현
- 데이터셋의 성별/연령대별 분포가 매우 불균형<br>
⇒ 차이가 없는 Class의 병합을 고려(특히, 7개의 Class로 분류된 연령대 중 큰 차이가 없는 Class가 있을 수 있음)<br>
⇒ 모델 학습 시, 리소스 제약 및 불균형 문제를 고려하여 언더 샘플링 수행 필요<br>
<br>

**[ 분석 결과/인사이트 ]**
- (가설) **줄임말, 고유 표현, 자음 또는 모음으로만 구성된 불완전 표현, 문장기호, 특수기호, 이모지 등의 사용행태**가 작성자의 인구 통계적 특성(성별/연령, Target Class)과 **중요한 상관관계**를 가질 수 있음
- Target Class별 표현의 변형/특수표현에 대한 차이가 존재하는지 파악 필요<br>
⇒ 분석을 위해 **언어적 특징에 대한 계량이 필요**

- 변형된 표현 및 특수한 표현이 제거되지 않도록 전처리 수행
<br><br>
